...
8. **Repository Integrity & Protected Surfaces (Governance Invariant / Candidate P-04.04 Primitive):** Forbidding any candidate patch from modifying Basebreak governance, verification harnesses, security policies, or evidence schemas.
...
| **THC** | Protected-Surface Mutation | Protected-surface manifest; normalized diff check; rejection of protected edits | P-04.04 | **IMPLEMENTED_PRIMITIVE (P-04.04 CANDIDATE)** |
...
| **THG** | Path Manipulation & Traversal | Normalized path validation; symlink inspection; workspace root enclosure | P-04.04 | **IMPLEMENTED_PRIMITIVE (P-04.04 CANDIDATE)** |
...
9. **Invariant 9 (Protected Surface Immutability):** A candidate patch cannot mutate protected verification contracts, governance rules, evidence schemas, or security policies to manufacture a PASS (Required Architecture / P-04.04 Candidate Primitive).
